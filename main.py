...
def process_script():
    ...
    for i in soup.select("p > b:nth-of-type(1)"):
        text = i.next_sibling.encode('utf-8')
        text = remove_utf_symbols(text).decode('utf-8')
        text = re.sub(r'\([^)]*\)', '', text)
        text = text.replace('\r','').replace('\n',' ')
        script.append((i.text,text.strip()))
...
def merge(subtitles,script,subs_words,script_words):
    ...
    while True:
        if count_i>len(subs_words) or count_j>len(script_words):
            break
        temp = []
        if subs_words[count_i]==script_words[count_j]:
            ...
            if script[count_jj][1].find(script_words[count_j]) != -1:
                temp.append(script[count_jj][0])
            else:
                count_jj+=1
                temp.append(script[count_jj][0])

            temp.append(subs_words[count_i])
            final.append(temp)
            count_i+=1
            count_j+=1
        else:
